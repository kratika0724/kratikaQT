...
class _AgentFilterBottomSheetState extends State<AgentFilterBottomSheet> {
  ...
  void _updateSuggestions(String query, bool isEmail) {
    ...
    final suggestions = provider.agents.map((agent) {
      if (isEmail) {
        return agent.email ?? '';
      } else {
        final fullName = [
  ...
  Widget _buildSuggestion(String value, TextEditingController controller, bool isEmail) {
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(value, style: regularTextStyle(fontSize: dimen13, color: AppColors.textBlack)),
        onTap: () {
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                return ChoiceChip(
                  backgroundColor: AppColors.grey100, // grey background when not selected
                  selectedColor: AppColors.primary,
                  checkmarkColor: AppColors.textWhite,
                  label: Text(item['label'] as String),
                  labelStyle: mediumTextStyle(fontSize: dimen13, color: _isActive == item['value'] ? AppColors.textWhite : AppColors.textBlack),
