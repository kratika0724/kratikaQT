    ...
    return widget.transactions.where((txn) {
      final matchesId = widget.filterTransactionId == null ||
          (txn.quintusTransactionId ?? '')
              .toLowerCase()
              .contains(widget.filterTransactionId!.toLowerCase());

      final matchesEmail = widget.filterEmail == null ||
          (txn.user.email ?? '')
              .toLowerCase()
              .contains(widget.filterEmail!.toLowerCase());

      final matchesStatus = widget.filterStatus == null ||
          (txn.status ?? '')
              .toLowerCase()
              .contains(widget.filterStatus!.toLowerCase());

      final matchesTransactionType = widget.filterTransactionType == null ||
          (txn.walletHistory.transactionType ?? '')
              .toLowerCase()
              .contains(widget.filterTransactionType!.toLowerCase());

      final matchesDateRange = widget.filterStartDate == null ||
              widget.filterEndDate == null
          ? true
          : (txn.createdAt.isAfter(
                  widget.filterStartDate!.subtract(const Duration(days: 1))) &&
              txn.createdAt.isBefore(
                  widget.filterEndDate!.add(const Duration(days: 1))));
